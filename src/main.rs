...
fn list_records_query(store: &StoreManager, query: &str, _i18n: &I18n, json: bool) -> Result<()> {
    ...
    if json {
        let out: Vec<serde_json::Value> = records.iter().map(|r| {
            serde_json::json!({
        ...
        for r in records {
            let ts = r.timestamp.with_timezone(&chrono::Local).format("%Y-%m-%d %H:%M:%S");
            if let Some(code) = r.short_code.as_deref() {
                println!("{} exit={} dur={}ms [code:{}] {}", ts, r.exit_code, r.duration_ms, code, r.command);
            } else {
                println!("{} exit={} dur={}ms {}", ts, r.exit_code, r.duration_ms, r.command);
            }
        }
    }
    Ok(())
...
#[allow(dead_code)]
fn list_records_file(store: &StoreManager, file_path: &std::path::Path, _i18n: &I18n, json: bool) -> Result<()> {
    let target_path = match std::fs::canonicalize(file_path) { Ok(p) => p, Err(_) => file_path.to_path_buf() };
    let mut records = store.get_all_records()?;
