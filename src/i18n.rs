...
impl I18n {
    pub fn new(lang: &str) -> Self {
        ...
        en.insert("help_run_diff_code".to_string(), "Show diff against an existing short code after run".to_string());
        ...
        en.insert("help_diff_max_shown".to_string(), "Selector viewport height (rows)".to_string());
        // Tips
        en.insert("help_tip_run_diff_code".to_string(), "Tip: dt run supports -d, --diff-code <CODE> — {0}".to_string());
        // Legacy alias notices
        en.insert("list_removed_notice".to_string(), "`dt list` is deprecated. Use `dt diff` instead. Opening selector...".to_string());
        // Config help
        ...
        en.insert("help_clean_search".to_string(), "Clean by command search".to_string());
        en.insert("help_clean_search_arg".to_string(), "Search query".to_string());
        ...
        en.insert("help_clean_backup".to_string(), "Backup matching records to current year's archive".to_string());
        en.insert("help_clean_dry_run".to_string(), "List matches without deleting".to_string());
        en.insert("help_ls".to_string(), "List records (non-interactive)".to_string());
        en.insert("help_ls_query".to_string(), "Optional query to filter (substring/subsequence)".to_string());
        en.insert("help_ls_json".to_string(), "Output JSON".to_string());
        // Dangerous command confirmations
        en.insert("confirm_clean_all_title".to_string(), "Dangerous operation: this will delete ALL history".to_string());
        en.insert("confirm_clean_all_prompt".to_string(), "Type YES to confirm: ".to_string());
        en.insert("clean_all_summary".to_string(), "Summary: {0} different commands, {1} total records".to_string());
        en.insert("confirm_clean_all_aborted".to_string(), "Aborted. No records were deleted.".to_string());
        // Generic deletion confirmations
        en.insert("confirm_delete_prompt".to_string(), "Type YES to confirm (or ALL to confirm all deletions this session): ".to_string());
        en.insert("delete_summary_query".to_string(), "About to delete {0} records matching: {1}".to_string());
        en.insert("delete_summary_file".to_string(), "About to delete {0} records related to file: {1}".to_string());
        en.insert("delete_nothing".to_string(), "No records matched; nothing to delete.".to_string());
        en.insert("dry_run_total".to_string(), "Dry-run total: {0} records".to_string());
        en.insert("backup_completed".to_string(), "Backed up records to index_{1}.json (total now: {0})".to_string());
        ...
        en.insert("diff_code_not_found".to_string(), "No record found with short code: {0}".to_string());
        ...
        en.insert("assigned_short_code".to_string(), "Short code: {0}".to_string());
        en.insert("hint_diff_with_code".to_string(), "Tip: run again with --diff-code={0} to compare".to_string());
        en.insert("select_command".to_string(), "Select a command to compare (Enter=open, Esc=quit):".to_string());
    ...
    en.insert("no_matches".to_string(), "No matches found".to_string());
        en.insert("navigate_hint".to_string(), "Navigation: j/k ↑/↓=move, Enter=select, Delete=clear, Esc=quit".to_string());
        en.insert("select_clean_command".to_string(), "Select a command to clean:".to_string());
        en.insert("select_clean_file".to_string(), "Select a file to clean:".to_string());
        ...
        zh.insert("help_diff_max_shown".to_string(), "选择器视口高度（行数）".to_string());
        // 提示
        zh.insert("help_tip_run_diff_code".to_string(), "提示：dt run 支持 -d, --diff-code <CODE> — {0}".to_string());
        // Config help
        ...
        zh.insert("help_run_diff_code".to_string(), "执行后与指定短码进行对比".to_string());
        // removed: help_clean_mode (not used)
        zh.insert("short_code_label".to_string(), "短码".to_string());
        zh.insert("help_clean_search".to_string(), "按命令搜索清理".to_string());
        zh.insert("help_clean_search_arg".to_string(), "搜索关键词".to_string());
        ...
        zh.insert("help_clean_backup".to_string(), "备份匹配记录到当年的归档文件".to_string());
        zh.insert("help_clean_dry_run".to_string(), "仅列出匹配结果，不执行删除".to_string());
        zh.insert("help_ls".to_string(), "简洁的非交互式列表".to_string());
        zh.insert("help_ls_query".to_string(), "可选的查询（子串/子序列）".to_string());
        zh.insert("help_ls_json".to_string(), "输出 JSON".to_string());
        // 危险命令确认
        zh.insert("confirm_clean_all_title".to_string(), "危险操作：将删除所有历史记录".to_string());
        zh.insert("confirm_clean_all_prompt".to_string(), "请输入 YES 以确认：".to_string());
        zh.insert("clean_all_summary".to_string(), "汇总: {0} 个不同命令，{1} 条记录".to_string());
        zh.insert("confirm_clean_all_aborted".to_string(), "已取消，未删除任何记录".to_string());
        // 通用删除确认
        zh.insert("confirm_delete_prompt".to_string(), "请输入 YES 确认（或输入 ALL 表示本次会话内不再提示）：".to_string());
        zh.insert("delete_summary_query".to_string(), "将删除匹配“{1}”的 {0} 条记录".to_string());
        zh.insert("delete_summary_file".to_string(), "将删除与文件相关的 {0} 条记录：{1}".to_string());
        zh.insert("delete_nothing".to_string(), "没有匹配记录，无需删除。".to_string());
        zh.insert("dry_run_total".to_string(), "试运行总计: {0} 条记录".to_string());
        zh.insert("backup_completed".to_string(), "已备份记录到 index_{1}.json（当前总数：{0}）".to_string());
        // 帮助区段标题
        ...
        zh.insert("diff_code_not_found".to_string(), "未找到短码为 {0} 的记录".to_string());
        ...
        zh.insert("hint_diff_with_code".to_string(), "提示: 再次使用 --diff-code={0} 可直接比较".to_string());
        zh.insert("select_command".to_string(), "选择一个命令进行比较（Enter进入，Esc退出）：".to_string());
        zh.insert("count_label".to_string(), "数量".to_string());
        zh.insert("latest_label".to_string(), "最新".to_string());

    // Interactive selection messages
    zh.insert("interactive_filter".to_string(), "过滤器 (输入模糊搜索，j/k 导航，Enter 选择，Delete 清空，Esc 退出)".to_string());
    ...
    zh.insert("navigate_hint".to_string(), "导航: j/k ↑/↓=移动, Enter=选择, Delete=清空, Esc=退出".to_string());
        zh.insert("select_clean_command".to_string(), "选择要清理的命令:".to_string());
        zh.insert("select_clean_file".to_string(), "选择要清理的文件:".to_string());
